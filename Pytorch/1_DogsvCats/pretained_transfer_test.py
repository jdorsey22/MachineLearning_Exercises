...
class CatsAndDogsDataset(Dataset):
    """ Dogs & Cats data Set """
    def __init__(self, REBUILD_DATA=False, IMG_SIZE=50, transform=None, PATH="C:\\Users\\Indiana\\Desktop\\Flowers\\flowers" ):
        ...
        self.DAISY = self.PATH + '\\daisy'
        self.DANDELION = self.PATH + '\\dandelion'
        self.ROSE = self.PATH + '\\rose'
        self.SUNFLOWER = self.PATH + '\\sunflower'
        self.TUPLIP = self.PATH + '\\tulip'
            ...
            for label in self.LABELS:
                for f in tqdm(os.listdir(label)):
                    try:
                        ...
                        img = img/256.0
                        self.data.append([torch.from_numpy(np.array(img)), np.eye(5)[self.LABELS[label]]])
...
class Net(nn.Module):
    def __init__(self):
        ...
        x = torch.rand(100, 100).view(-1, 1, 50, 50)
        self.convs(x)
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        for (imgs, labels) in tqdm(train_data):
            net.train()
            net.zero_grad()
            output = net(imgs.float().view(-1, 3, 50, 50).to(device))
        ...
        with torch.no_grad():
            ...
            for (imgs, labels) in tqdm(val_data):
                ...
                net_out = net(imgs.float().view(-1, 3, 50, 50).to(device))[0]
